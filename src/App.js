...
function App() {
  ...
  const addInstance = async () => {
    try {
        const response = await axios.post(`${API_BASE_URL}instances/`, newInstance);
        ...
        console.error('Failed to add instance:', error.response?.data || error.message);
    }
};
  ...
  const fetchInstances = async (year, semester) => {
    const response = await axios.get(
      `${API_BASE_URL}instances/${year}/${semester}`
    );
    ...
    <Container maxWidth="md">
      ...
      <Box my={4} display="flex" alignItems="center">
        <Select
          ...
          onChange={(e) =>
            setNewInstance({ ...newInstance, courseId: e.target.value })
          }
          displayEmpty
          fullWidth
        ...
        <TextField
          ...
          onChange={(e) =>
            setNewInstance({ ...newInstance, year: e.target.value })
          }
          style={{ marginRight: "10px" }}
        />
        <TextField
          ...
          onChange={(e) =>
            setNewInstance({ ...newInstance, semester: e.target.value })
          }
          style={{ marginRight: "10px" }}
        />
      ...
      <Box my={4} display="flex" alignItems="center">
        ...
        <Button
          variant="contained"
          color="primary"
          onClick={() => fetchInstances(selectedYear, selectedSemester)}
        >
          List instances
        </Button>
      </Box>
